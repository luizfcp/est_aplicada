...
\hspace*{1.25 cm}Ao decorrer das análises foram utilizados testes de hipóteses estatísticos, metodologia estatística que nos auxilia a tomar decisões sobre uma ou mais populações baseado na informação obtida da amostra, são eles:
...
\label{tab:inflamacao}
\end{table}

\hspace*{1.25 cm}Para as variáveis ICAM e PCR pode-se observar pelas Figuras \ref{fig:ICAM}, \ref{fig:PCR} e pela Tabela \ref{tab:inflamacao} uma diminuição de sua média e desvio padrão para a coleta de dados depois do RETP. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP. 

\hspace*{1.25 cm}Já para as variáveis IL6, TNFa e VCAM pode-se observar pelas Figuras \ref{fig:IL6}, \ref{fig:TNFa}, \ref{fig:VCAM} e pela Tabela \ref{tab:inflamacao} uma diminuição de sua média mas um crescimento no desvio padrão para a coleta de dados depois do RETP. Diferente da variável Albumina que pela Figura \ref{fig:ALB} e pela Tabela \ref{tab:inflamacao} pode-se observar que teve um aumento em sua média mas uma diminuição no desvio padrão. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP. 
...
\hspace*{1.25 cm}Para a variável AMB pode-se observar pela Figura \ref{fig:AMB} e pela Tabela \ref{tab:antropometricos} uma diminuição de sua média e desvio padrão para a coleta de dados depois do RETP. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP. 

\hspace*{1.25 cm}Já para as variáveis IMC e Massa Magra pode-se observar pelas Figuras \ref{fig:IMC}, \ref{fig:massa_magra} e pela Tabela \ref{tab:antropometricos} um aumento de sua média, porém um aumento e uma dimunuição, respectivamente, ppara o desvio padrão para a coleta de dados depois do RETP. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP. 
...
\hspace*{1.25 cm}Para as variáveis SL10 e Torque Extensor Esquerdo pode-se observar pelas Figuras \ref{fig:SL10}, \ref{fig:Torque Extensor Esquerdo} e pela Tabela \ref{tab:cf} uma diminuição de sua média e desvio padrão para a coleta de dados depois do RETP. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP para a variável SL10, já para a variável Torque Extensor Esquerdo obteve-se, pelo teste-t, que não houve diferença entre as médias de antes e depois do RETP.

\hspace*{1.25 cm}Já para as variáveis SL60, Torque Extensor Direito, Torque Flexor Direito e Torque Flexor Esquerdo pode-se observar pelas Figuras \ref{fig:SL60}, \ref{fig:Torque Extensor Direito}, \ref{fig:Torque Flexor Direito}, \ref{fig:Torque Flexor Esquerdo} e pela Tabela \ref{tab:cf} um aumento de sua média e desvio padrão para a coleta de dados depois do RETP. Pelo teste-t, obtivemos de fato que houve diferença entre as médias de antes e depois do RETP. 
...
\hspace*{1.25 cm}O foco do estudo foi verificar se os pacientes submetidos à hemodiálise apresentaram melhoras nas variáveis com respeito a inflamação e perda de energia proteica (PEW)  depois do programa de treinamento de exercícios resistidos (RETP). 
...
\hspace*{1.25 cm}Sobre o PEW, analisando os dados obtidos, foi notado uma redução dos pacientes que possuíam essa perda de energia proteica.
