...
meses_12 <- base_gr %>% 
  bind_rows() %>% 
  gather(mes, value, -ano, -GR) %>% distinct(mes)


# pintar março, abril, maio de cores diferentes
p1 <- 
  base_gr %>% 
  bind_rows() %>% 
  ...
  ungroup() %>% 
  mutate(mes = factor(mes, levels = meses$mes)) %>% 
...
base_gr %>% 
  ...
  theme_minimal() +
  coord_flip() +
  geom_text(aes(label = media %>% round), nudge_y = -8500, color = "white", size = 3) +
  labs(x = "", y = "Média", title = "Média de nascimento por mês (2003-2017)")
