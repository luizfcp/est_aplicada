...
$P(Y \leq 80) \approx P(Z \leq -0.85) = 1 - P(Z \leq 0.85) = 0,1977$

Logo, p-valor = $2 \times P(Y \leq 80) = 0,3954$. Assim, com base no nível de significância de 5%, não rejeitamos $H_0$, ou seja, há evidências de que 30% das pessoas têm problemas de audição.
...
ggplot(data.frame(x), aes(x, y)) +
  geom_line() +
  geom_vline(xintercept = 0.17, color = "red", linetype = "dashed") +
  geom_hline(yintercept = 0.05, color = "red", linetype = "dashed") +
  labs(x = "p", y = expression(alpha)) +
  theme_minimal()
```
...
```{r}
binom.test(10, 15, 0.9, alternative = "two.sided")
```
